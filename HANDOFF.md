...
> **问题实证（2026-08-29）**：文件本身**无结构缺陷**——netCDF4/h5py 读写一切正常
> （dimensions xdim=128/ydim=128/tdim=512/const=1；u/v (512,128,128)；坐标变量
> long_name/units 齐全；与 cavity 等参考数据集逐字段同构）；用户用 **ParaView 打开**
> 显示为 1×1×1 退化网格（# of Cells: 1、Bounds 全部 0 to 0 (delta: 0)、Data Arrays
> 仅余 alpha/beta）并报 `vtkPVImageSliceMapper: Incorrect dimensionality`——即
> **ParaView 走了 Image reader 路径而非 rectilinear grid reader**（读器选择/兼容问题，
> 可尝试 ParaView 显式选 NetCDF/NetCDF CF reader；非数据内容错误）。用户保持移出
> 决策：常规复核工具（ParaView）不可用 → 数据可信度受损；且该数据集贡献最小
> （正格占比 8.3% 为 7 者最低、τ=0.006 近 doublegyre 微尺度、无固体）。如需救回
> （CF 元数据补齐/转格式）属独立任务。Kaggle Dataset A 无需重传——config root 即
